...
export interface IImageWebSocketService {
...
class ImageWebsocketService implements IImageWebSocketService {
    ...
    connect(projectId: string) {
        if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
            this.socket = new WebSocket(`${import.meta.env.VITE_WS_BASE_URL}/ws/image?projectId=${projectId}`);
            ...
            this.socket.onmessage = (event: MessageEvent) => {
                ...
                if (!!data) {
                    ...
                    if (types.includes(EventTypes.image_generate_start)) {
                        emitter?.emit(EventTypes.image_generate_start, data);
                    }
                    if (types.includes(EventTypes.image_generate_progress)) {
                        emitter?.emit(EventTypes.image_generate_progress, data);
                    }
                    if (types.includes(EventTypes.image_generate_result)) {
                        emitter?.emit(EventTypes.image_generate_result, data);
                    }
...
export function createImageWebsocketService() {
    return new ImageWebsocketService();
}
